...
Live-version discovery identifies its HTTP client as `RedFolder.Deployment/1.0`. The endpoint rejected the generic Python client identity with HTTP 403 during the first promotion preflight, before any production upload. The explicit deployment identity was verified successfully through the complete read-only artifact preflight. Lookup failures now report a sanitized HTTP status, timeout, connectivity or JSON error; no response body, URL or credentials are logged. A failed lookup still blocks promotion. Redirect rejection, HTTPS validation and the 15-second request timeout remain enforced.
